...
public class WorkshopStoryBrowserPanel : Panel
{
    ...
    private float CreateTransitionRuleCard(StoryNodeTransitionDocument transition, int ruleIndex, float y)
    {
        ...
        float height = 140f + Mathf.Max(1, conditionCount) * 38f;
        ...
        if (transition.isEnd)
        {
            CreateText("End Transition Notice", card, "结束剧情后直接关闭，不使用场景转场", 14,
                TextAnchor.MiddleLeft, HintColor, new Vector2(0f, 1f), new Vector2(0f, 1f),
                new Vector2(14f, rowY - 36f), new Vector2(360f, 26f));
        }
        else
        {
            CreateTransitionEffectControls(card, transition, rowY - 36f);
        }
        return height;
    }
    ...
    private float CreateDefaultFlowCard(float y)
    {
        ...
        if (explicitDefault != null)
        {
            ...
            if (explicitDefault.isEnd)
            {
                CreateText("End Transition Notice", card, "结束剧情后直接关闭，不使用场景转场", 14,
                    TextAnchor.MiddleLeft, HintColor, new Vector2(0f, 1f), new Vector2(0f, 1f),
                    new Vector2(14f, -72f), new Vector2(360f, 26f));
            }
            else
            {
                CreateTransitionEffectControls(card, explicitDefault, -72f);
            }
        ...
        {
            string transitionDescription = string.IsNullOrWhiteSpace(targetNodeId)
                ? "结束剧情后直接关闭，不使用场景转场"
                : "转场：沿用目标场景设置";
            CreateText("Default Transition", card, transitionDescription, 14, TextAnchor.MiddleLeft, HintColor,
                new Vector2(0f, 1f), new Vector2(0f, 1f), new Vector2(14f, -72f), new Vector2(270f, 26f));
    ...
    private void CreateTransitionEffectControls(
        RectTransform card,
        StoryNodeTransitionDocument transition,
        float y)
    {
        CreateText("Transition Effect Label", card, "转场", 14, TextAnchor.MiddleLeft, HintColor,
            new Vector2(0f, 1f), new Vector2(0f, 1f), new Vector2(14f, y), new Vector2(52f, 26f));
        ...
        Dropdown dropdown = CreateDropdown(card, new Vector2(72f, y), new Vector2(300f, 26f), optionIndex =>
        {
            if (optionIndex < 0 || optionIndex >= types.Length)
                return;
            float duration = transition?.transition?.normalizedDuration ?? .35f;
            if (!controller.UpdateSelectedNodeTransitionEffect(
                    transition.transitionId, types[optionIndex], duration, out string error))
            {
                Hintbox.OpenHintboxWithContent(error, 16);
                return;
            }
        ...
        if (dropdown != null)
        {
            ...
            string type = transition?.transition?.normalizedType ?? "inherit";
            dropdown.SetValueWithoutNotify(Mathf.Max(0, Array.IndexOf(types, type)));
            SetSelectorValueText(CreateSelectorValueText(dropdown), dropdown, "沿用目标场景");
        }

        float currentDuration = transition?.transition?.normalizedDuration ?? .35f;
        CreateActionButton(card, "时长 " + currentDuration.ToString("0.00") + "秒",
            new Vector2(382f, y), new Vector2(112f, 26f),
            () => CycleTransitionDuration(transition), TextAnchor.UpperLeft);
    }

    private void CycleTransitionDuration(StoryNodeTransitionDocument transition)
    {
        if (transition == null)
            return;

        float[] durations = { .25f, .35f, .5f, .75f, 1f };
        float current = transition.transition?.normalizedDuration ?? .35f;
        ...
        string type = transition.transition?.normalizedType ?? "inherit";
        if (!controller.UpdateSelectedNodeTransitionEffect(transition.transitionId, type, next, out string error))
        {
            Hintbox.OpenHintboxWithContent(error, 16);
            return;
        }
